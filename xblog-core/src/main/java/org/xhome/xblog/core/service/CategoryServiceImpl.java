...
public class CategoryServiceImpl implements CategoryService {
	...
	public int addCategory(User oper, Category category) {
		...
		if (categoryDAO.isCategoryExists(category)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to add category {}, but it's already exists", name);
			}
		...
		short r = categoryDAO.addCategory(category) == 1 ? Status.SUCCESS : Status.ERROR;
	...
	public int updateCategory(User oper, Category category) {
		...
		if (!this.beforeCategoryManage(oper, Action.UPDATE, category)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update category {}[{}], but it's blocked", name, id);
			}
			
			this.logManage(name, Action.UPDATE, null, Status.BLOCKED, oper);
			this.afterCategoryManage(oper, Action.UPDATE, Status.BLOCKED, category);
			return Status.BLOCKED;
		}
		...
		if (old == null) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update category {}[{}], but it's not exists", name, id);
			}
			
			this.logManage(name, Action.UPDATE, id, Status.NOT_EXISTS, oper);
			this.afterCategoryManage(oper, Action.UPDATE, Status.NOT_EXISTS, category);
		...
		if (!old.getVersion().equals(category.getVersion())) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update category {}[{}], but version not match", oldName, id);
			}
			
			this.logManage(oldName, Action.UPDATE, id, Status.VERSION_NOT_MATCH, oper);
			this.afterCategoryManage(oper, Action.UPDATE, Status.VERSION_NOT_MATCH, category);
		...
		if (status == Status.NO_UPDATE || status == Status.LOCK) {
			if (logger.isDebugEnabled()) {
				logger.debug("it's not allowed to update category {}[{}]", oldName, id);
			}
			
			this.logManage(oldName, Action.UPDATE, id, status, oper);
			this.afterCategoryManage(oper, Action.UPDATE, Status.EXISTS, category);
			return status;
		}
		
		if (!oldName.equals(name) && categoryDAO.isCategoryExists(category)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update category {}[{}] to {}, but it's exists", oldName, id, name);
			}
		...
		short r  = categoryDAO.updateCategory(category) == 1 ? Status.SUCCESS : Status.ERROR;
		if (r == Status.SUCCESS) {
	...
	public int lockCategory(User oper, Category category) {
		...
		if (!this.beforeCategoryManage(oper, Action.LOCK, category)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to lock category {}[{}], but it's blocked", name, id);
			}
			
			this.logManage(name, Action.LOCK, null, Status.BLOCKED, oper);
			this.afterCategoryManage(oper, Action.LOCK, Status.BLOCKED, category);
			return Status.BLOCKED;
		}
		
		short r = categoryDAO.lockCategory(category) == 1 ? Status.SUCCESS : Status.ERROR;
	...
	public int unlockCategory(User oper, Category category) {
		...
		if (!this.beforeCategoryManage(oper, Action.UNLOCK, category)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to unlock category {}[{}], but it's blocked", name, id);
			}
			
			this.logManage(name, Action.UNLOCK, null, Status.BLOCKED, oper);
			this.afterCategoryManage(oper, Action.UNLOCK, Status.BLOCKED, category);
			return Status.BLOCKED;
		}
		
		short r = categoryDAO.unlockCategory(category) == 1 ? Status.SUCCESS : Status.ERROR;
	...
	public int removeCategory(User oper, Category category) {
		...
		if (!this.beforeCategoryManage(oper, Action.REMOVE, category)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to remove category {}[{}], but it's blocked", name, id);
			}
			
			this.logManage(name, Action.REMOVE, null, Status.BLOCKED, oper);
			this.afterCategoryManage(oper, Action.REMOVE, Status.BLOCKED, category);
			return Status.BLOCKED;
		}
	...
	public int deleteCategory(User oper, Category category) {
		...
		if (!this.beforeCategoryManage(oper, Action.DELETE, category)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to delete category {}[{}], but it's blocked", name, id);
			}
			
			this.logManage(name, Action.DELETE, null, Status.BLOCKED, oper);
			this.afterCategoryManage(oper, Action.DELETE, Status.BLOCKED, category);
			return Status.BLOCKED;
		}
	...
	public boolean isCategoryExists(User oper, Category category) {
		...
		if (!this.beforeCategoryManage(oper, Action.IS_EXISTS, category)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge exists of category {}, but it's blocked", name);
			}
			
			this.logManage(name, Action.IS_EXISTS, null, Status.BLOCKED, oper);
			this.afterCategoryManage(oper, Action.IS_EXISTS, Status.BLOCKED, category);
			return false;
		}
		...
		this.logManage(name, Action.IS_EXISTS, category.getId(), Status.SUCCESS, oper);
		this.afterCategoryManage(oper, Action.IS_EXISTS, Status.SUCCESS, category);
		return e;
	}
	...
	public boolean isCategoryUpdateable(User oper, Category category) {
		...
		if (!this.beforeCategoryManage(oper, Action.IS_UPDATEABLE, category)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge updateable of category {}[{}], but it's blocked", name, id);
			}
			
			this.logManage(name, Action.IS_UPDATEABLE, null, Status.BLOCKED, oper);
			this.afterCategoryManage(oper, Action.IS_UPDATEABLE, Status.BLOCKED, category);
			return false;
		}
		...
		this.afterCategoryManage(oper, Action.IS_UPDATEABLE, Status.SUCCESS, category);
		return e;
	}
	...
	public boolean isCategoryLocked(User oper, Category category) {
		...
		if (!this.beforeCategoryManage(oper, Action.IS_LOCKED, category)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge locked of category {}[{}], but it's blocked", name, id);
			}
			
			this.logManage(name, Action.IS_LOCKED, null, Status.BLOCKED, oper);
			this.afterCategoryManage(oper, Action.IS_LOCKED, Status.BLOCKED, category);
			return false;
		}
		...
		this.afterCategoryManage(oper, Action.IS_LOCKED, Status.SUCCESS, category);
		return e;
	}
	...
	public boolean isCategoryRemoveable(User oper, Category category) {
		...
		if (!this.beforeCategoryManage(oper, Action.IS_REMOVEABLE, category)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge removeable of category {}[{}], but it's blocked", name, id);
			}
			
			this.logManage(name, Action.IS_REMOVEABLE, null, Status.BLOCKED, oper);
			this.afterCategoryManage(oper, Action.IS_REMOVEABLE, Status.BLOCKED, category);
			return false;
		}
		...
		this.afterCategoryManage(oper, Action.IS_REMOVEABLE, Status.SUCCESS, category);
		return e;
	}
	...
	public boolean isCategoryDeleteable(User oper, Category category) {
		...
		if (!this.beforeCategoryManage(oper, Action.IS_DELETEABLE, category)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge deleteable of category {}[{}], but it's blocked", name, id);
			}
			
			this.logManage(name, Action.IS_DELETEABLE, null, Status.BLOCKED, oper);
			this.afterCategoryManage(oper, Action.IS_DELETEABLE, Status.BLOCKED, category);
			return false;
		}
		...
		this.afterCategoryManage(oper, Action.IS_DELETEABLE, Status.SUCCESS, category);
		return e;
	}
	...
	public Category getCategory(User oper, long id) {
		if (!this.beforeCategoryManage(oper, Action.GET, null, id)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to get category of id {}, but it's blocked", id);
			}
	...
	public Category getCategory(User oper, String name) {
		if (!this.beforeCategoryManage(oper, Action.GET, null, name)) {
			...
			this.afterCategoryManage(oper, Action.GET, Status.BLOCKED, null, name);
			return null;
		}
		...
		this.logManage(name, Action.GET, id, Status.SUCCESS, oper);
		this.afterCategoryManage(oper, Action.GET, Status.SUCCESS, category, name);
		return category;
	}
	
	@Override
	public List<Category> getCategorys(User oper) {
		return getCategorys(oper, null);
	}
	...
	public List<Category> getCategorys(User oper, QueryBase query) {
		if (!this.beforeCategoryManage(oper, Action.QUERY, null, query)) {
			...
			this.afterCategoryManage(oper, Action.QUERY, Status.BLOCKED, null, query);
			return null;
		}
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug("query categorys with parameters {}", query.getParameters());
			} else {
		...
		this.afterCategoryManage(oper, Action.QUERY, Status.SUCCESS, null, query);
		return results;
	}
	
	@Override
	public long countCategorys(User oper) {
		return countCategorys(oper, null);
	}
	...
	public long countCategorys(User oper, QueryBase query) {
		if (!this.beforeCategoryManage(oper, Action.COUNT, null, query)) {
			...
			this.afterCategoryManage(oper, Action.COUNT, Status.BLOCKED, null, query);
			return -1;
		}
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug("count categorys with parameters {} of {}", query.getParameters(), c);
			} else {
		...
		this.afterCategoryManage(oper, Action.COUNT, Status.SUCCESS, null, query);
		return c;
	}
	
	private void logManage(String content, Short action, Long obj, Short status, User oper) {
		ManageLog manageLog = new ManageLog(content, action, ManageLog.TYPE_CATAGORY, obj, oper == null ? null : oper.getId());
		manageLog.setStatus(status);
		manageLogService.logManage(manageLog);
	}
	
	private boolean beforeCategoryManage(User oper, short action, Category category, Object ...args) {
		if (categoryManageListeners != null) {
			for (CategoryManageListener listener : categoryManageListeners) {
				if (!listener.beforeCategoryManage(oper, action, category, args)) {
					return false;
				}
			}
		}
	...
	private void afterCategoryManage(User oper, short action, short result, Category category, Object ...args) {
		if (categoryManageListeners != null) {
			for (CategoryManageListener listener : categoryManageListeners) {
				listener.afterCategoryManage(oper, action, result, category, args);
			}
		}
	}
	...
	public void setCategoryManageListeners(List<CategoryManageListener> categoryManageListeners) {
	...
	public void registerCategoryManageListener(CategoryManageListener categoryManageListener) {
		if (categoryManageListeners == null) {
