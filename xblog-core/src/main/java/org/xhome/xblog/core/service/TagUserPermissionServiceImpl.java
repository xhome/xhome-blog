...
public class TagUserPermissionServiceImpl implements TagUserPermissionService {
	...
	@Override
	public int addTagUserPermission(User oper,
		...
		if (!this.beforeTagUserPermissionManage(oper, Action.ADD,
				tagUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to add tag {}[{}] permission {} for user {}[{}], but it's blocked",
						tagName, tagId, permission, userName, userId);
			}

			this.logManage(mstr, Action.ADD, null, Status.BLOCKED, oper);
			this.afterTagUserPermissionManage(oper, Action.ADD, Status.BLOCKED,
					tagUserPermission);
		...
		if (tagUserPermissionDAO.isTagUserPermissionExists(tagUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to add tag {}[{}] permission {} for user {}[{}], but it's already exists",
						tagName, tagId, permission, userName, userId);
			}

			this.logManage(mstr, Action.ADD, null, Status.EXISTS, oper);
			this.afterTagUserPermissionManage(oper, Action.ADD, Status.EXISTS,
					tagUserPermission);
		...
		short r = tagUserPermissionDAO.addTagUserPermission(tagUserPermission) == 1 ? Status.SUCCESS
				: Status.ERROR;

		if (logger.isDebugEnabled()) {
			if (r == Status.SUCCESS) {
				logger.debug(
						"success to add tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			} else {
				logger.debug(
						"fail to add tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			}
		}

		this.logManage(mstr, Action.ADD, tagUserPermission.getId(), r, oper);
		this.afterTagUserPermissionManage(oper, Action.ADD, r,
				tagUserPermission);
		return r;
	}
	...
	@Transactional(isolation = Isolation.READ_COMMITTED, rollbackFor = Throwable.class)
	@Override
	public int updateTagUserPermission(User oper,
			TagUserPermission tagUserPermission) {
		...
		if (!this.beforeTagUserPermissionManage(oper, Action.UPDATE,
				tagUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to update tag {}[{}] permission {} for user {}[{}], but it's blocked",
						tagName, tagId, permission, userName, userId);
			}

			this.logManage(mstr, Action.UPDATE, null, Status.BLOCKED, oper);
			this.afterTagUserPermissionManage(oper, Action.UPDATE,
		...
		if (old == null) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to update tag {}[{}] permission {} for user {}[{}], but it's not exists",
						tagName, tagId, permission, userName, userId);
			}
		...
		if (!old.getVersion().equals(tagUserPermission.getVersion())) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to update tag {}[{}] permission {} for user {}[{}], but version not match",
						tagName, tagId, permission, userName, userId);
			}
			...
			this.logManage(mstr, Action.UPDATE, id, Status.VERSION_NOT_MATCH,
					oper);
			this.afterTagUserPermissionManage(oper, Action.UPDATE,
					Status.VERSION_NOT_MATCH, tagUserPermission);
		...
		if (status == Status.NO_UPDATE || status == Status.LOCK) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"it's not allowed to update tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			}

			this.logManage(mstr, Action.UPDATE, id, status, oper);
			this.afterTagUserPermissionManage(oper, Action.UPDATE,
					Status.EXISTS, tagUserPermission);
			return status;
		}
		...
		short r = tagUserPermissionDAO
				.updateTagUserPermission(tagUserPermission) == 1 ? Status.SUCCESS
				: Status.ERROR;
		if (r == Status.SUCCESS) {
		...
		if (logger.isDebugEnabled()) {
			if (r == Status.SUCCESS) {
				logger.debug(
						"success to update tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			} else {
				logger.debug(
						"fail to update tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			}
		}

		this.logManage(mstr, Action.UPDATE, id, r, oper);
		this.afterTagUserPermissionManage(oper, Action.UPDATE, r,
				tagUserPermission);
		return r;
	}
	...
	@Transactional(isolation = Isolation.READ_COMMITTED, rollbackFor = Throwable.class)
	@Override
	public int lockTagUserPermission(User oper,
			TagUserPermission tagUserPermission) {
		long id = tagUserPermission.getId();
		Tag tag = tagUserPermission.getTag();
		User user = tagUserPermission.getUser();
		...
		if (!this.beforeTagUserPermissionManage(oper, Action.LOCK,
				tagUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to lock tag {}[{}] permission {} for user {}[{}], but it's blocked",
						tagName, tagId, permission, userName, userId);
			}

			this.logManage(mstr, Action.LOCK, null, Status.BLOCKED, oper);
			this.afterTagUserPermissionManage(oper, Action.LOCK,
					Status.BLOCKED, tagUserPermission);
			return Status.BLOCKED;
		}

		short r = tagUserPermissionDAO.lockTagUserPermission(tagUserPermission) == 1 ? Status.SUCCESS
				: Status.ERROR;

		if (logger.isDebugEnabled()) {
			if (r == Status.SUCCESS) {
				logger.debug(
						"success to lock tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			} else {
				logger.debug(
						"fail to lock tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			}
		}

		this.logManage(mstr, Action.LOCK, id, r, oper);
		this.afterTagUserPermissionManage(oper, Action.LOCK, r,
				tagUserPermission);
		return r;
	}
	...
	public int unlockTagUserPermission(User oper,
		...
		if (!this.beforeTagUserPermissionManage(oper, Action.UNLOCK,
				tagUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to unlock tag {}[{}] permission {} for user {}[{}], but it's blocked",
						tagName, tagId, permission, userName, userId);
			}

			this.logManage(mstr, Action.UNLOCK, null, Status.BLOCKED, oper);
			this.afterTagUserPermissionManage(oper, Action.UNLOCK,
					Status.BLOCKED, tagUserPermission);
			return Status.BLOCKED;
		}

		short r = tagUserPermissionDAO
				.unlockTagUserPermission(tagUserPermission) == 1 ? Status.SUCCESS
				: Status.ERROR;

		if (logger.isDebugEnabled()) {
			if (r == Status.SUCCESS) {
				logger.debug(
						"success to unlock tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			} else {
				logger.debug(
						"fail to unlock tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			}
		}

		this.logManage(mstr, Action.UNLOCK, id, r, oper);
		this.afterTagUserPermissionManage(oper, Action.UNLOCK, r,
				tagUserPermission);
		return r;
	}
	...
	public int deleteTagUserPermission(User oper,
		...
		if (!this.beforeTagUserPermissionManage(oper, Action.DELETE,
				tagUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to delete tag {}[{}] permission {} for user {}[{}], but it's blocked",
						tagName, tagId, permission, userName, userId);
			}

			this.logManage(mstr, Action.DELETE, null, Status.BLOCKED, oper);
			this.afterTagUserPermissionManage(oper, Action.DELETE,
					Status.BLOCKED, tagUserPermission);
			return Status.BLOCKED;
		}
		...
		if (tagUserPermissionDAO
				.isTagUserPermissionDeleteable(tagUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug("delete tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			}
			...
			if (logger.isDebugEnabled()) {
				logger.debug(
						"tag {}[{}] permission {} for user {}[{}] isn't deleteable",
						tagName, tagId, permission, userName, userId);
			}
			r = Status.NO_DELETE;
		}
	...
	public boolean isTagUserPermissionExists(User oper,
		...
		if (!this.beforeTagUserPermissionManage(oper, Action.IS_EXISTS,
				tagUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to juge exists of tag {}[{}] permission {} for user {}[{}], but it's blocked",
						tagName, tagId, permission, userName, userId);
			}

			this.logManage(mstr, Action.IS_EXISTS, null, Status.BLOCKED, oper);
			this.afterTagUserPermissionManage(oper, Action.IS_EXISTS,
					Status.BLOCKED, tagUserPermission);
			return false;
		}

		boolean e = tagUserPermissionDAO
				.isTagUserPermissionExists(tagUserPermission);

		if (logger.isDebugEnabled()) {
			if (e) {
				logger.debug(
						"exists of tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			} else {
				logger.debug(
						"not exists of tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			}
		}

		this.logManage(mstr, Action.IS_EXISTS, tagUserPermission.getId(),
				Status.SUCCESS, oper);
		this.afterTagUserPermissionManage(oper, Action.IS_EXISTS,
	...
	@Override
	public boolean isTagUserPermissionUpdateable(User oper,
			TagUserPermission tagUserPermission) {
		long id = tagUserPermission.getId();
		Tag tag = tagUserPermission.getTag();
		User user = tagUserPermission.getUser();
		...
		if (!this.beforeTagUserPermissionManage(oper, Action.IS_UPDATEABLE,
				tagUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to juge updateable of tag {}[{}] permission {} for user {}[{}], but it's blocked",
						tagName, tagId, permission, userName, userId);
			}

			this.logManage(mstr, Action.IS_UPDATEABLE, null, Status.BLOCKED,
					oper);
			this.afterTagUserPermissionManage(oper, Action.IS_UPDATEABLE,
					Status.BLOCKED, tagUserPermission);
			return false;
		}

		boolean e = tagUserPermissionDAO
				.isTagUserPermissionUpdateable(tagUserPermission);

		if (logger.isDebugEnabled()) {
			if (e) {
				logger.debug(
						"tag {}[{}] permission {} for user {}[{}] is updateable",
						tagName, tagId, permission, userName, userId);
			} else {
				logger.debug(
						"tag {}[{}] permission {} for user {}[{}] isn't updateable",
						tagName, tagId, permission, userName, userId);
			}
		}
	...
	public boolean isTagUserPermissionLocked(User oper,
		...
		if (!this.beforeTagUserPermissionManage(oper, Action.IS_LOCKED,
				tagUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to juge locked of tag {}[{}] permission {} for user {}[{}], but it's blocked",
						tagName, tagId, permission, userName, userId);
			}
		...
		boolean e = tagUserPermissionDAO
				.isTagUserPermissionLocked(tagUserPermission);

		if (logger.isDebugEnabled()) {
			if (e) {
				logger.debug(
						"tag {}[{}] permission {} for user {}[{}] is locked",
						tagName, tagId, permission, userName, userId);
			} else {
				logger.debug(
						"tag {}[{}] permission {} for user {}[{}] isn't locked",
						tagName, tagId, permission, userName, userId);
			}
		}
	...
	public boolean isTagUserPermissionDeleteable(User oper,
		...
		if (!this.beforeTagUserPermissionManage(oper, Action.IS_DELETEABLE,
				tagUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to juge deleteable of tag {}[{}] permission {} for user {}[{}], but it's blocked",
						tagName, tagId, permission, userName, userId);
			}

			this.logManage(mstr, Action.IS_DELETEABLE, null, Status.BLOCKED,
					oper);
			this.afterTagUserPermissionManage(oper, Action.IS_DELETEABLE,
					Status.BLOCKED, tagUserPermission);
			return false;
		}
		...
		if (logger.isDebugEnabled()) {
			if (e) {
				logger.debug(
						"tag {}[{}] permission {} for user {}[{}] is deleteable",
						tagName, tagId, permission, userName, userId);
			} else {
				logger.debug(
						"tag {}[{}] permission {} for user {}[{}] isn't deleteable",
	...
	public TagUserPermission getTagUserPermission(User oper, long id) {
		if (!this.beforeTagUserPermissionManage(oper, Action.GET, null, id)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to get tag user permission of id {}, but it's blocked",
						id);
			}

			this.logManage("" + id, Action.GET, null, Status.BLOCKED, oper);
			this.afterTagUserPermissionManage(oper, Action.GET, Status.BLOCKED,
					null, id);
			return null;
		}

		TagUserPermission tagUserPermission = tagUserPermissionDAO
				.queryTagUserPermission(id);
		...
		if (logger.isDebugEnabled()) {
			if (tagUserPermission != null) {
				...
				String tagName = tag != null ? tag.getName() : "", userName = user != null ? user
						.getName() : "";
				int permission = tagUserPermission.getPermission();
				mstr = tagName + "(" + tagId + ")" + ", " + userName + "("
						+ userId + ")";
				logger.debug("get tag {}[{}] permission {} for user {}[{}]",
						tagName, tagId, permission, userName, userId);
			} else {
		...
		this.afterTagUserPermissionManage(oper, Action.GET, Status.SUCCESS,
				tagUserPermission, id);
	...
	public List<TagUserPermission> getTagUserPermissions(User oper,
			QueryBase query) {
		if (!this
				.beforeTagUserPermissionManage(oper, Action.QUERY, null, query)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to query tag user permissions, but it's blocked");
			}

			this.logManage(null, Action.QUERY, null, Status.BLOCKED, oper);
			this.afterTagUserPermissionManage(oper, Action.QUERY,
					Status.BLOCKED, null, query);
			return null;
		}

		List<TagUserPermission> results = tagUserPermissionDAO
				.queryTagUserPermissions(query);
		if (query != null) {
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug("query tag user permissions with parameters {}",
						query.getParameters());
			} else {
		...
		this.afterTagUserPermissionManage(oper, Action.QUERY, Status.SUCCESS,
				null, query);
		return results;
	}
	...
	public long countTagUserPermissions(User oper, QueryBase query) {
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug(
						"count tag user permissions with parameters {} of {}",
		...
		this.afterTagUserPermissionManage(oper, Action.COUNT, Status.SUCCESS,
				null, query);
		return c;
	}
	...
	private boolean beforeTagUserPermissionManage(User oper, short action,
		...
		if (tagUserPermissionManageListeners != null) {
			for (TagUserPermissionManageListener listener : tagUserPermissionManageListeners) {
				if (!listener.beforeTagUserPermissionManage(oper, action,
						tagUserPermission, args)) {
					return false;
				}
			}
		}
	...
	private void afterTagUserPermissionManage(User oper, short action,
			short result, TagUserPermission tagUserPermission, Object... args) {
		if (tagUserPermissionManageListeners != null) {
			for (TagUserPermissionManageListener listener : tagUserPermissionManageListeners) {
				listener.afterTagUserPermissionManage(oper, action, result,
						tagUserPermission, args);
			}
		}
	}

	public void setTagUserPermissionDAO(
			TagUserPermissionDAO tagUserPermissionDAO) {
	...
	public void setTagUserPermissionManageListeners(
			List<TagUserPermissionManageListener> tagUserPermissionManageListeners) {
	...
	public void registerTagUserPermissionManageListener(
			TagUserPermissionManageListener tagUserPermissionManageListener) {
		if (tagUserPermissionManageListeners == null) {
