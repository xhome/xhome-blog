 ...
 */
@Service
public class ArticleUserPermissionServiceImpl implements
		ArticleUserPermissionService {
	...
	public int addArticleUserPermission(User oper,
		...
		if (!this.beforeArticleUserPermissionManage(oper, Action.ADD,
				articleUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to add article {}[{}] permission {} for user {}[{}], but it's blocked",
						articleTitle, articleId, permission, userName, userId);
			}

			this.logManage(mstr, Action.ADD, null, Status.BLOCKED, oper);
			this.afterArticleUserPermissionManage(oper, Action.ADD,
					Status.BLOCKED, articleUserPermission);
			return Status.BLOCKED;
		}

		if (articleUserPermissionDAO
				.isArticleUserPermissionExists(articleUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to add article {}[{}] permission {} for user {}[{}], but it's already exists",
						articleTitle, articleId, permission, userName, userId);
			}

			this.logManage(mstr, Action.ADD, null, Status.EXISTS, oper);
			this.afterArticleUserPermissionManage(oper, Action.ADD,
					Status.EXISTS, articleUserPermission);
		...
		short r = articleUserPermissionDAO
				.addArticleUserPermission(articleUserPermission) == 1 ? Status.SUCCESS
				: Status.ERROR;

		if (logger.isDebugEnabled()) {
			if (r == Status.SUCCESS) {
				logger.debug(
						"success to add article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			} else {
				logger.debug(
						"fail to add article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			}
		}

		this.logManage(mstr, Action.ADD, articleUserPermission.getId(), r, oper);
		this.afterArticleUserPermissionManage(oper, Action.ADD, r,
				articleUserPermission);
		return r;
	}
	...
	public int updateArticleUserPermission(User oper,
		...
		if (!this.beforeArticleUserPermissionManage(oper, Action.UPDATE,
				articleUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to update article {}[{}] permission {} for user {}[{}], but it's blocked",
						articleTitle, articleId, permission, userName, userId);
			}

			this.logManage(mstr, Action.UPDATE, null, Status.BLOCKED, oper);
			this.afterArticleUserPermissionManage(oper, Action.UPDATE,
					Status.BLOCKED, articleUserPermission);
			return Status.BLOCKED;
		}

		ArticleUserPermission old = articleUserPermissionDAO
				.queryArticleUserPermission(id);

		if (old == null) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to update article {}[{}] permission {} for user {}[{}], but it's not exists",
						articleTitle, articleId, permission, userName, userId);
			}

			this.logManage(mstr, Action.UPDATE, id, Status.NOT_EXISTS, oper);
			this.afterArticleUserPermissionManage(oper, Action.UPDATE,
					Status.NOT_EXISTS, articleUserPermission);
		...
		if (!old.getVersion().equals(articleUserPermission.getVersion())) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to update article {}[{}] permission {} for user {}[{}], but version not match",
						articleTitle, articleId, permission, userName, userId);
			}
			...
			this.logManage(mstr, Action.UPDATE, id, Status.VERSION_NOT_MATCH,
					oper);
			this.afterArticleUserPermissionManage(oper, Action.UPDATE,
					Status.VERSION_NOT_MATCH, articleUserPermission);
		...
		if (status == Status.NO_UPDATE || status == Status.LOCK) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"it's not allowed to update article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			}
		...
		short r = articleUserPermissionDAO
				.updateArticleUserPermission(articleUserPermission) == 1 ? Status.SUCCESS
				: Status.ERROR;
		if (r == Status.SUCCESS) {
		...
		if (logger.isDebugEnabled()) {
			if (r == Status.SUCCESS) {
				logger.debug(
						"success to update article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			} else {
				logger.debug(
						"fail to update article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			}
		}

		this.logManage(mstr, Action.UPDATE, id, r, oper);
		this.afterArticleUserPermissionManage(oper, Action.UPDATE, r,
				articleUserPermission);
		return r;
	}
	...
	public int lockArticleUserPermission(User oper,
		...
		if (!this.beforeArticleUserPermissionManage(oper, Action.LOCK,
				articleUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to lock article {}[{}] permission {} for user {}[{}], but it's blocked",
						articleTitle, articleId, permission, userName, userId);
			}

			this.logManage(mstr, Action.LOCK, null, Status.BLOCKED, oper);
			this.afterArticleUserPermissionManage(oper, Action.LOCK,
					Status.BLOCKED, articleUserPermission);
			return Status.BLOCKED;
		}

		short r = articleUserPermissionDAO
				.lockArticleUserPermission(articleUserPermission) == 1 ? Status.SUCCESS
				: Status.ERROR;

		if (logger.isDebugEnabled()) {
			if (r == Status.SUCCESS) {
				logger.debug(
						"success to lock article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			} else {
				logger.debug(
						"fail to lock article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			}
		}

		this.logManage(mstr, Action.LOCK, id, r, oper);
		this.afterArticleUserPermissionManage(oper, Action.LOCK, r,
				articleUserPermission);
		return r;
	}
	...
	public int unlockArticleUserPermission(User oper,
		...
		if (!this.beforeArticleUserPermissionManage(oper, Action.UNLOCK,
				articleUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to unlock article {}[{}] permission {} for user {}[{}], but it's blocked",
						articleTitle, articleId, permission, userName, userId);
			}

			this.logManage(mstr, Action.UNLOCK, null, Status.BLOCKED, oper);
			this.afterArticleUserPermissionManage(oper, Action.UNLOCK,
					Status.BLOCKED, articleUserPermission);
			return Status.BLOCKED;
		}

		short r = articleUserPermissionDAO
				.unlockArticleUserPermission(articleUserPermission) == 1 ? Status.SUCCESS
				: Status.ERROR;

		if (logger.isDebugEnabled()) {
			if (r == Status.SUCCESS) {
				logger.debug(
						"success to unlock article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			} else {
				logger.debug(
						"fail to unlock article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			}
		}

		this.logManage(mstr, Action.UNLOCK, id, r, oper);
		this.afterArticleUserPermissionManage(oper, Action.UNLOCK, r,
				articleUserPermission);
		return r;
	}
	...
	public int deleteArticleUserPermission(User oper,
		...
		if (!this.beforeArticleUserPermissionManage(oper, Action.DELETE,
				articleUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to delete article {}[{}] permission {} for user {}[{}], but it's blocked",
						articleTitle, articleId, permission, userName, userId);
			}

			this.logManage(mstr, Action.DELETE, null, Status.BLOCKED, oper);
			this.afterArticleUserPermissionManage(oper, Action.DELETE,
					Status.BLOCKED, articleUserPermission);
			return Status.BLOCKED;
		}
		...
		short r = Status.SUCCESS;
		if (articleUserPermissionDAO
				.isArticleUserPermissionDeleteable(articleUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"delete article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			}
			articleUserPermissionDAO
					.deleteArticleUserPermission(articleUserPermission);
		} else {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"article {}[{}] permission {} for user {}[{}] isn't deleteable",
						articleTitle, articleId, permission, userName, userId);
			}
			r = Status.NO_DELETE;
		}
		...
		this.afterArticleUserPermissionManage(oper, Action.DELETE, r,
				articleUserPermission);
		return r;
	}
	...
	public int deleteArticleUserPermissions(User oper,
		...
		int r = Status.SUCCESS;
		for (ArticleUserPermission articleUserPermission : articleUserPermissions) {
			r = this.deleteArticleUserPermission(oper, articleUserPermission);
			if (r != Status.SUCCESS) {
				throw new RuntimeException(
						"fail to delete ArticleUserPermission ["
								+ articleUserPermission.getId() + "]");
			}
		}
		return r;
	...
	public boolean isArticleUserPermissionExists(User oper,
		...
		if (!this.beforeArticleUserPermissionManage(oper, Action.IS_EXISTS,
				articleUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to juge exists of article {}[{}] permission {} for user {}[{}], but it's blocked",
						articleTitle, articleId, permission, userName, userId);
			}

			this.logManage(mstr, Action.IS_EXISTS, null, Status.BLOCKED, oper);
			this.afterArticleUserPermissionManage(oper, Action.IS_EXISTS,
					Status.BLOCKED, articleUserPermission);
			return false;
		}
		...
		if (logger.isDebugEnabled()) {
			if (e) {
				logger.debug(
						"exists of article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			} else {
				logger.debug(
						"not exists of article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			}
		}

		this.logManage(mstr, Action.IS_EXISTS, articleUserPermission.getId(),
				Status.SUCCESS, oper);
		this.afterArticleUserPermissionManage(oper, Action.IS_EXISTS,
	...
	public boolean isArticleUserPermissionUpdateable(User oper,
		...
		if (!this.beforeArticleUserPermissionManage(oper, Action.IS_UPDATEABLE,
				articleUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to juge updateable of article {}[{}] permission {} for user {}[{}], but it's blocked",
						articleTitle, articleId, permission, userName, userId);
			}

			this.logManage(mstr, Action.IS_UPDATEABLE, null, Status.BLOCKED,
					oper);
			this.afterArticleUserPermissionManage(oper, Action.IS_UPDATEABLE,
					Status.BLOCKED, articleUserPermission);
			return false;
		}

		boolean e = articleUserPermissionDAO
				.isArticleUserPermissionUpdateable(articleUserPermission);

		if (logger.isDebugEnabled()) {
			if (e) {
				logger.debug(
						"article {}[{}] permission {} for user {}[{}] is updateable",
						articleTitle, articleId, permission, userName, userId);
			} else {
				logger.debug(
						"article {}[{}] permission {} for user {}[{}] isn't updateable",
						articleTitle, articleId, permission, userName, userId);
			}
		}
	...
	public boolean isArticleUserPermissionLocked(User oper,
		...
		if (!this.beforeArticleUserPermissionManage(oper, Action.IS_LOCKED,
				articleUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to juge locked of article {}[{}] permission {} for user {}[{}], but it's blocked",
						articleTitle, articleId, permission, userName, userId);
			}
		...
		boolean e = articleUserPermissionDAO
				.isArticleUserPermissionLocked(articleUserPermission);

		if (logger.isDebugEnabled()) {
			if (e) {
				logger.debug(
						"article {}[{}] permission {} for user {}[{}] is locked",
						articleTitle, articleId, permission, userName, userId);
			} else {
				logger.debug(
						"article {}[{}] permission {} for user {}[{}] isn't locked",
						articleTitle, articleId, permission, userName, userId);
			}
		}
	...
	public boolean isArticleUserPermissionDeleteable(User oper,
		...
		if (!this.beforeArticleUserPermissionManage(oper, Action.IS_DELETEABLE,
				articleUserPermission)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to juge deleteable of article {}[{}] permission {} for user {}[{}], but it's blocked",
						articleTitle, articleId, permission, userName, userId);
			}

			this.logManage(mstr, Action.IS_DELETEABLE, null, Status.BLOCKED,
					oper);
			this.afterArticleUserPermissionManage(oper, Action.IS_DELETEABLE,
					Status.BLOCKED, articleUserPermission);
			return false;
		}
		...
		if (logger.isDebugEnabled()) {
			if (e) {
				logger.debug(
						"article {}[{}] permission {} for user {}[{}] is deleteable",
						articleTitle, articleId, permission, userName, userId);
			} else {
				logger.debug(
						"article {}[{}] permission {} for user {}[{}] isn't deleteable",
	...
	public ArticleUserPermission getArticleUserPermission(User oper, long id) {
		if (!this.beforeArticleUserPermissionManage(oper, Action.GET, null, id)) {
			if (logger.isDebugEnabled()) {
				logger.debug(
						"try to get article user permission of id {}, but it's blocked",
						id);
			}

			this.logManage("" + id, Action.GET, null, Status.BLOCKED, oper);
			this.afterArticleUserPermissionManage(oper, Action.GET,
					Status.BLOCKED, null, id);
			return null;
		}

		ArticleUserPermission articleUserPermission = articleUserPermissionDAO
				.queryArticleUserPermission(id);
		...
		if (logger.isDebugEnabled()) {
			if (articleUserPermission != null) {
				...
				String articleTitle = article != null ? article.getTitle() : "", userName = user != null ? user
						.getName() : "";
				int permission = articleUserPermission.getPermission();
				mstr = articleTitle + "(" + articleId + ")" + ", " + userName
						+ "(" + userId + ")";
				logger.debug(
						"get article {}[{}] permission {} for user {}[{}]",
						articleTitle, articleId, permission, userName, userId);
			} else {
				logger.debug("article user permission of id {} is not exists",
						id);
			}
		}

		this.logManage(mstr, Action.GET, id, Status.SUCCESS, oper);
		this.afterArticleUserPermissionManage(oper, Action.GET, Status.SUCCESS,
				articleUserPermission, id);
	...
	public List<ArticleUserPermission> getArticleUserPermissions(User oper,
			QueryBase query) {
		if (!this.beforeArticleUserPermissionManage(oper, Action.QUERY, null,
				query)) {
			if (logger.isDebugEnabled()) {
			...
			this.afterArticleUserPermissionManage(oper, Action.QUERY,
					Status.BLOCKED, null, query);
			return null;
		}

		List<ArticleUserPermission> results = articleUserPermissionDAO
				.queryArticleUserPermissions(query);
		if (query != null) {
			query.setResults(results);
			long total = articleUserPermissionDAO
					.countArticleUserPermissions(query);
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug(
						"query article user permissions with parameters {}",
						query.getParameters());
			} else {
	...
	public long countArticleUserPermissions(User oper, QueryBase query) {
		if (!this.beforeArticleUserPermissionManage(oper, Action.COUNT, null,
				query)) {
			if (logger.isDebugEnabled()) {
			...
			this.afterArticleUserPermissionManage(oper, Action.COUNT,
					Status.BLOCKED, null, query);
			return -1;
		}
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug(
						"count article user permissions with parameters {} of {}",
	...
	private boolean beforeArticleUserPermissionManage(User oper, short action,
		...
		if (articleUserPermissionManageListeners != null) {
			for (ArticleUserPermissionManageListener listener : articleUserPermissionManageListeners) {
				if (!listener.beforeArticleUserPermissionManage(oper, action,
						articleUserPermission, args)) {
					return false;
				}
			}
		}
	...
	private void afterArticleUserPermissionManage(User oper, short action,
			short result, ArticleUserPermission articleUserPermission,
			Object... args) {
		if (articleUserPermissionManageListeners != null) {
			for (ArticleUserPermissionManageListener listener : articleUserPermissionManageListeners) {
				listener.afterArticleUserPermissionManage(oper, action, result,
						articleUserPermission, args);
			}
		}
	}

	public void setArticleUserPermissionDAO(
			ArticleUserPermissionDAO articleUserPermissionDAO) {
	...
	public void setArticleUserPermissionManageListeners(
			List<ArticleUserPermissionManageListener> articleUserPermissionManageListeners) {
	...
	public void registerArticleUserPermissionManageListener(
			ArticleUserPermissionManageListener articleUserPermissionManageListener) {
		if (articleUserPermissionManageListeners == null) {
			articleUserPermissionManageListeners = new ArrayList<ArticleUserPermissionManageListener>();
		}
		articleUserPermissionManageListeners
				.add(articleUserPermissionManageListener);
	}
