...
public class RecordServiceImpl implements RecordService {
	...
	public int logRecord(Record record) {
		...
		int r = recordDAO.addRecord(record) == 1 ? Status.SUCCESS
				: Status.ERROR;

		if (logger.isDebugEnabled()) {
			if (r == Status.SUCCESS) {
				logger.debug("success to add record for {}", record.getUser()
						.getName());
			} else {
				logger.debug("fail to add record for {}", record.getUser()
						.getName());
			}
		}
	...
	public List<Record> getRecords(User oper, QueryBase query) {
		if (!this.beforeRecordManage(oper, Action.QUERY, null, query)) {
			...
			this.afterRecordManage(oper, Action.QUERY, Status.BLOCKED, null,
					query);
			return null;
		}
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug("query user records with parameters {}",
						query.getParameters());
			} else {
	...
	public long countRecords(User oper, QueryBase query) {
		if (!this.beforeRecordManage(oper, Action.COUNT, null, query)) {
			...
			this.afterRecordManage(oper, Action.COUNT, Status.BLOCKED, null,
					query);
			return -1;
		}
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug("count user records with parameters {} of {}",
	...
	private void logManage(String content, Short action, Long obj,
		...
		ManageLog manageLog = new ManageLog(ManageLog.MANAGE_LOG_XBLOG,
				content, action, ManageLogType.RECORD, obj, oper == null ? null
						: oper.getId());
		manageLog.setStatus(status);
		manageLogService.logManage(manageLog);
	}

	private boolean beforeRecordManage(User oper, short action, Record record,
			Object... args) {
	...
	private void afterRecordManage(User oper, short action, short result,
	...
	public void setRecordManageListeners(
			List<RecordManageListener> recordManageListeners) {
	...
	public void registerRecordManageListener(
			RecordManageListener recordManageListener) {
		if (recordManageListeners == null) {
