...
public class TagServiceImpl implements TagService {
	...
	public int updateTag(User oper, Tag tag) {
		...
		if (!this.beforeTagManage(oper, Action.UPDATE, tag)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update tag {}[{}], but it's blocked", name, id);
			}
		...
		if (old == null) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update tag {}[{}], but it's not exists", name, id);
			}
		...
		if (!old.getVersion().equals(tag.getVersion())) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update tag {}[{}], but version not match", oldName, id);
			}
			
			this.logManage(oldName, Action.UPDATE, id, Status.VERSION_NOT_MATCH, oper);
			this.afterTagManage(oper, Action.UPDATE, Status.VERSION_NOT_MATCH, tag);
		...
		if (status == Status.NO_UPDATE || status == Status.LOCK) {
			if (logger.isDebugEnabled()) {
				logger.debug("it's not allowed to update tag {}[{}]", oldName, id);
			}
		...
		if (!oldName.equals(name) && tagDAO.isTagExists(tag)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to update tag {}[{}] to {}, but it's exists", oldName, id, name);
			}
	...
	public int lockTag(User oper, Tag tag) {
		...
		if (!this.beforeTagManage(oper, Action.LOCK, tag)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to lock tag {}[{}], but it's blocked", name, id);
			}
	...
	public int unlockTag(User oper, Tag tag) {
		...
		if (!this.beforeTagManage(oper, Action.UNLOCK, tag)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to unlock tag {}[{}], but it's blocked", name, id);
			}
	...
	public int removeTag(User oper, Tag tag) {
		...
		if (!this.beforeTagManage(oper, Action.REMOVE, tag)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to remove tag {}[{}], but it's blocked", name, id);
			}
	...
	public int deleteTag(User oper, Tag tag) {
		...
		if (!this.beforeTagManage(oper, Action.DELETE, tag)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to delete tag {}[{}], but it's blocked", name, id);
			}
	...
	public boolean isTagExists(User oper, Tag tag) {
		...
		if (!this.beforeTagManage(oper, Action.IS_EXISTS, tag)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge exists of tag {}, but it's blocked", name);
			}
		...
		this.logManage(name, Action.IS_EXISTS, tag.getId(), Status.SUCCESS, oper);
	...
	public boolean isTagUpdateable(User oper, Tag tag) {
		...
		if (!this.beforeTagManage(oper, Action.IS_UPDATEABLE, tag)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge updateable of tag {}[{}], but it's blocked", name, id);
			}
	...
	public boolean isTagLocked(User oper, Tag tag) {
		...
		if (!this.beforeTagManage(oper, Action.IS_LOCKED, tag)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge locked of tag {}[{}], but it's blocked", name, id);
			}
	...
	public boolean isTagRemoveable(User oper, Tag tag) {
		...
		if (!this.beforeTagManage(oper, Action.IS_REMOVEABLE, tag)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge removeable of tag {}[{}], but it's blocked", name, id);
			}
	...
	public boolean isTagDeleteable(User oper, Tag tag) {
		...
		if (!this.beforeTagManage(oper, Action.IS_DELETEABLE, tag)) {
			if (logger.isDebugEnabled()) {
				logger.debug("try to juge deleteable of tag {}[{}], but it's blocked", name, id);
			}
	...
	@Override
	public List<Tag> getTags(User oper) {
		return getTags(oper, null);
	}
	...
	public List<Tag> getTags(User oper, QueryBase query) {
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug("query tags with parameters {}", query.getParameters());
			} else {
	...
	@Override
	public long countTags(User oper) {
		return countTags(oper, null);
	}
	...
	public long countTags(User oper, QueryBase query) {
		...
		if (logger.isDebugEnabled()) {
			if (query != null) {
				logger.debug("count tags with parameters {} of {}", query.getParameters(), c);
			} else {
	...
	private void logManage(String content, Short action, Long obj, Short status, User oper) {
		ManageLog manageLog = new ManageLog(content, action, ManageLog.TYPE_TAG, obj, oper == null ? null : oper.getId());
		manageLog.setStatus(status);
		manageLogService.logManage(manageLog);
	}
	
	private boolean beforeTagManage(User oper, short action, Tag tag, Object ...args) {
	...
	private void afterTagManage(User oper, short action, short result, Tag tag, Object ...args) {
		if (tagManageListeners != null) {
			for (TagManageListener listener : tagManageListeners) {
