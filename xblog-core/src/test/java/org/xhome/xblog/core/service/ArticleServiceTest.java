...
public class ArticleServiceTest extends AbstractTest {
	...
	public ArticleServiceTest() {
		...
		((ArticleServiceImpl)articleService).registerArticleManageListener(new TestArticleManageListener());
		((ArticleServiceImpl)articleService).registerArticleTagManageListener(new TestArticleTagManageListener());
	...
	public void testCountArticles() {
		logger.info("{}", articleService.countArticles(oper));
	...
	public void testGetArticles() {
		List<Article> articles = articleService.getArticles(oper);
		printArticle(articles);
		
		QueryBase query = new QueryBase();
		query.addParameter("name", "test");
		articles = articleService.getArticles(oper, query);
		printArticle(articles);
	}
	...
	public void testIsArticleTagUpdateable(){
		...
		logger.info("{}", articleService.isArticleTagUpdateable(oper, article, tag));
	}
	...
	public void testIsArticleTagDeleteable(){
		Article article = articleService.getArticle(oper, 1L);
		Tag tag = tagService.getTag(oper, 1L);
		logger.info("{}", articleService.isArticleTagDeleteable(oper, article, tag));
	}
