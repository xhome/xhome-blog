...
public class TestTagManageListener implements TagManageListener {
	...
	public boolean beforeTagManage(User oper, short action, Tag tag,
			Object... args) {
		logger.debug("TEST BEFORE TAG MANAGE LISTENER {} {} {}", oper.getName(), action, tag != null ? tag.getName() : "NULL");
		return true;
	}

	@Override
	public void afterTagManage(User oper, short action, short result,
			Tag tag, Object... args) {
		logger.debug("TEST AFTER TAG MANAGE LISTENER {} {} {}", oper.getName(), action, tag != null ? tag.getName() : "NULL");
	}
