...
public class TestCommentManageListener implements CommentManageListener {
	...
	@Override
	public boolean beforeCommentManage(User oper, short action, Comment comment,
			Object... args) {
		logger.debug("TEST BEFORE COMMENT MANAGE LISTENER {} {} {}", oper.getName(), action, comment != null ? comment.getContent() : "NULL");
		return true;
	}
	...
	public void afterCommentManage(User oper, short action, short result,
		...
		logger.debug("TEST AFTER COMMENT MANAGE LISTENER {} {} {}", oper.getName(), action, comment != null ? comment.getContent() : "NULL");
	}
