 ...
 * @description 
 */
public class ArticlePermissionRoleValidator extends Validator {
	...
	public ArticlePermissionRoleValidator() {
	...
	public boolean validate(Object target, Errors errors) {
		ArticleRolePermission permission = (ArticleRolePermission) target;
		Role role = permission == null ? null : permission.getRole();
		if (role == null) {
			errors.rejectValue(FIELD_ROLE, CODE_ROLE_EMPTY, validationConfig.getConfig(BlogValidatorConfig.ARTICLE_PERMISSION_ROLE_EMPTY_MESSAGE));
			return false;
		}
