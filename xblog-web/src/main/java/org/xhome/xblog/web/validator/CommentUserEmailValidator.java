 ...
 * @describe 评论用户的邮箱校验
 */
public class CommentUserEmailValidator extends Validator {
	...
	public boolean validate(Object target, Errors errors) {
		...
		String userEmail = comment == null ? null : comment.getUserEmail();
		if (StringUtils.isEmpty(userEmail)) {
			errors.rejectValue(
					FIELD_USER_EMAIL,
					CODE_USER_EMAIL_EMPTY,
					validationConfig
							.getConfig(BlogValidatorConfig.COMMENT_USER_EMAIL_EMPTY_MESSAGE));
		} else {
			int size = userEmail.length();
			int min = Integer
					.parseInt(validationConfig
							.getConfig(BlogValidatorConfig.COMMENT_USER_EMAIL_SIZE_MIN)), max = Integer
					.parseInt(validationConfig
							.getConfig(BlogValidatorConfig.COMMENT_USER_EMAIL_SIZE_MAX));
			if (size < min || size > max) {
				errors.rejectValue(
						FIELD_USER_EMAIL,
						CODE_USER_EMAIL_SIZE,
						validationConfig
								.getConfig(BlogValidatorConfig.COMMENT_USER_EMAIL_SIZE_MESSAGE));
			} else {
				if (!userEmail
						.matches(validationConfig
								.getConfig(BlogValidatorConfig.COMMENT_USER_EMAIL_PATTERN_REGEXP))) {
					errors.rejectValue(
							FIELD_USER_EMAIL,
							CODE_USER_EMAIL_PATTERN,
							validationConfig
									.getConfig(BlogValidatorConfig.COMMENT_USER_EMAIL_PATTERN_MESSAGE));
				} else {
					return true;
				}
			}
