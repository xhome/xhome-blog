 ...
 * @description 
 */
public class CategoryPermissionRoleValidator extends Validator {
	...
	public CategoryPermissionRoleValidator() {
	...
	public boolean validate(Object target, Errors errors) {
		CategoryRolePermission permission = (CategoryRolePermission) target;
		Role role = permission == null ? null : permission.getRole();
		if (role == null) {
			errors.rejectValue(FIELD_ROLE, CODE_ROLE_EMPTY, validationConfig.getConfig(BlogValidatorConfig.CATEGORY_PERMISSION_ROLE_EMPTY_MESSAGE));
			return false;
		}
