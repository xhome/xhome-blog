 ...
 * @description 
 */
public class CategoryPermissionUserValidator extends Validator {
	...
	public CategoryPermissionUserValidator() {
	...
	public boolean validate(Object target, Errors errors) {
		CategoryUserPermission permission = (CategoryUserPermission) target;
		User user = permission == null ? null : permission.getUser();
		if (user == null) {
			errors.rejectValue(FIELD_USER, CODE_USER_EMPTY, validationConfig.getConfig(BlogValidatorConfig.CATEGORY_PERMISSION_USER_EMPTY_MESSAGE));
			return false;
		}
